...
class EmperialGramianMatrix:
    def __init__(self, x0, n_pert, c, x_max, t, dt_meas, t_meas, uex_func, PN_func, D_func, SI_func, threshold, SI_est = None):
        self.x0 = x0
        self.n_pert = n_pert
        self.i_max = 2 ** n_pert
    ...
    def outputs(self, x0_i, x0):
        ...
        if self.SI_est is None:
            self.y_0_k =  icing_true.ICINGTrue().simulate(x0, self.t[0], self.t[-1], self.t, self.uex_func, self.PN_func, self.D_func, self.SI_func)[0][::self.dt_meas]
            ...
            self.y_0_k =  icing_true.ICINGTrue().simulate(x0[:-1], self.t[0], self.t[-1], self.t, self.uex_func, self.PN_func, self.D_func, self.SI_func)[0][::self.dt_meas]

            for i in range(self.i_max):
                SI_func = utils.gen_SI_func(SI_const=x0_i[-1,i])
                y_i[:,i] = ICINGTrue.simulate(x0_i[:-1,i], self.t[0], self.t[-1], self.t, self.uex_func, self.PN_func, self.D_func, SI_func)[0][::self.dt_meas]
