...
class ICINGTrue:
    ...
    def __icing_odes(self, t, y, uex_func, PN_func, D_func, SI_func):
        ...
        dGdt = (
            -self.params["pG"] * G
            - insulin_effect
            + (P_t + self.params["EGP"] - self.params["CNS"]) / self.params["VG"]
        )

        dQdt = self.params["nI"] * (I - Q) - self.params["nC"] * Q / (
            1.0 + self.params["alphaG"] * Q
        )

        dIdt = (
            -self.params["nK"] * I
            - self.params["nL"] * I / (1.0 + self.params["alphaI"] * I)
            - self.params["nI"] * (I - Q)
            + uex / self.params["VI"]
            + (1.0 - self.params["xL"]) * uen / self.params["VI"]
        )

        dP1dt = -self.params["d1"] * P1 + D

        dP2dt = (
            -min(self.params["d2"] * P2, self.params["Pmax"]) + self.params["d1"] * P1
        )
    ...
    def simulate(self, y0, t_start, t_end, t_eval, uex_func, PN_func, D_func, SI_func):
        ...
        sol = integrate.solve_ivp(
            ...
            (t_start, t_end),
            y0,
            args=(uex_func, PN_func, D_func, SI_func),
            t_eval=t_eval,
            rtol=1e-6,
            atol=1e-9,
        )
        ...
        P = [
            min(self.params["d2"] * sol.y[4][idx], self.params["Pmax"]) + PN_func(ti)
    ...
    def plot_sim(self, simulation=None):
        ...
        ax[1].plot(
            simulation["t"], simulation["I"], label="Insulin (mU/L)", color="orange"
        )
        ax[1].plot(
            simulation["t"],
            simulation["uex"],
        ...
        effective_interstitial_insulin = simulation["Q"] / (
            1.0 + self.params["alphaG"] * simulation["Q"]
        )
        ax[2].plot(
            simulation["t"],
        ...
        P_from_gut = np.minimum(
            self.params["d2"] * simulation["P2"], self.params["Pmax"]
        )
        ...
        ax[4].plot(
            simulation["t"], simulation["SI"], label="SI (L/mU/min)", color="red"
        )
